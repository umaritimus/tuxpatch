...
#   include tuxpatch
class tuxpatch (
    Hash[Integer,String] $tuxedo_patches = lookup('tuxedo_patches')
) {
    if ($facts['operatingsystem'] == 'windows') {
        ...
        $tuxedo_patches.each | Integer $patch_index, String $patch_path | {
          if (!empty($patch_path)) {
            ...
            notice("${pkgtemp} = ${patch_path} ; ${opatch_path}")
              ...
              command   =>  Sensitive(@("EOT")),
                  ...
                  Start-Process `
                    ...
                    -ArgumentList @( `
                      ...
                      ${regsubst("\'${pkgtemp}1\'" ,'/', '\\\\', 'G')}, `
                      "/E /PURGE /NOCOPY /MOVE /NFL /NDL /NJH /NJS > nul" `
                    ) `
                    -Wait `
                    -NoNewWindow | Out-Null

                  Get-Item -Path ${regsubst("\'${pkgtemp}1\'" ,'/', '\\\\', 'G')} -ErrorAction SilentlyContinue | Remove-Item -Force -Recurse
                |-EOT
              provider  => powershell,
              logoutput => true,
